...
docker buildx bake --no-cache --load --progress=plain -f "$SCRIPT_DIR/autoware-universe/docker-bake.hcl" \
    ...
    --set "devel.tags=gitlab.fixstars.com:5005/pasta/autoware:$rosdistro-latest$image_name_suffix" \
    --set "prebuilt.tags=gitlab.fixstars.com:5005/pasta/autoware:$rosdistro-latest-prebuilt$image_name_suffix" \
    "${targets[@]}"
set +x
